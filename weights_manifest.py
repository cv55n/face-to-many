...
class WeightsManifest:
    ...
    def _download_updated_weights_manifest(self):
        if not os.path.exists(UPDATED_WEIGHTS_MANIFEST_PATH):
            print(
                f"baixando o manifest atualizado dos pesos de {UPDATED_WEIGHTS_MANIFEST_URL}"
            )
            ...
            print(
                f"baixar {UPDATED_WEIGHTS_MANIFEST_URL} levou: {(time.time() - start):.2f}s"
            )
        else:
            print("arquivo manifest atualizado dos pesos já existente")
            ...
            if key in original_manifest:
                for item in updated_manifest[key]:
                    if item not in original_manifest[key]:
                        print(f"adicionando {item} em {key}")
